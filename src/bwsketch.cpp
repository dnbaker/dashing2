...
BigWigSketchResult bw2sketch(std::string path, const Dashing2Options &opts) {
...
#ifdef _OPENMP
    #pragma omp parallel for schedule(dynamic) reduction(+:total_weight)
#endif
    ...
    for(size_t i = 0; i < ids.size(); ++i) {
        ...
        }
        {
            ...
            long double inc;
            if(fss.size()) inc = fss[tid].getcard();
            else if(opss.size()) inc = opss[tid].getcard();
            else if(bmhs.size()) inc = bmhs[tid].total_weight();
            else if(pmhs.size()) inc = pmhs[tid].total_weight();
            else throw std::runtime_error("Unexpected sketching method");
            total_weight += inc;
